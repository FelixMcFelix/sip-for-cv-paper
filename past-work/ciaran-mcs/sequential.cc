...
namespace
{
    ...
    struct SIP
    {
        ...
        {
            bool distance_filter = pattern.has_pos && target.has_pos;
            ...
            while (true) {
                ...
                for (unsigned p = 0 ; p < pattern.size() ; ++p) {
                    ...
                    for (unsigned t = 0 ; t < target.size() ; ++t) {
                        ...
                        // if the input data has attached positions
                        const int MAX_MATCH_DIST = 5;
                        constexpr int MAX_MATCH_DIST_SQUARE = MAX_MATCH_DIST * MAX_MATCH_DIST;

                        if (distance_filter)
                        {
                            auto l = pattern.get_pos(p);
                            auto r = target.get_pos(t);
                            auto dx = l.first - r.first;
                            auto dy = l.second - r.second;

                            ok = (dx*dx + dy*dy) <= MAX_MATCH_DIST_SQUARE;
                        }
