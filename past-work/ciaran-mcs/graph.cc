...
auto Graph::add_pos(unsigned node, unsigned x, unsigned y) -> void
{
    _coord[node] = std::pair<std::uint16_t, std::uint16_t>(x, y);
    has_pos = true;
}

auto Graph::get_pos(unsigned node) const -> std::pair<std::uint16_t, std::uint16_t>
{
    return _coord[node];
}
