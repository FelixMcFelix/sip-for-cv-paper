...
class Graph
{
    public:
        ...
        using PositionList = std::vector<std::pair<std::uint16_t, std::uint16_t>>;
    ...
    public:
        ...
        auto add_pos(unsigned node, unsigned x, unsigned y) -> void;

        auto get_pos(unsigned node) const -> std::pair<std::uint16_t, std::uint16_t>;
