...
%	\KwResult{An attributed multigraph $\mathcal{G}=...$}
	...
	\Begin{
		$V \leftarrow \lbrace\rbrace$\;
		$E \leftarrow []$\;
		$B \leftarrow I$ after padding, thresholding and binarisation\;
		...
		\For{$i\leftarrow 1$; $i \le n_{components}$; $i$\texttt{++}}{
			$PathPoints = [(x,y,l) \in Keyps \cup Ends : l = i]$\; 
			\uIf{$|PathPoints| > 1$}{$start_i \leftarrow$ any $p \in PathPoints$}
			\Else{$start \leftarrow$ topmost, leftmost $(x,y,l)$ with $l=C[x,y]=i$}
			
			$Paths \leftarrow$ \texttt{Traverse}($start, PathPoints, S$)\;
			\ForEach{$(p_{start}, p_{end}) \in Paths$}{
				$Splits \leftarrow$ \texttt{IPAN}($S, p_{start}, p_{end}$)\;
%				Then Modify Edge and vertex sets. Note---where are verts added in code?\;
				\For{$j \leftarrow 0$; $j < |Splits|-1$; $j$\texttt{++}}{
					$p_1 \leftarrow Splits[j]$\;
					$p_2 \leftarrow Splits[j+1]$\;
					
					$d \leftarrow dist(p_1, p_2)$\;
					$t \leftarrow trace\_dist(S, p_{start}, p_{end}, p_1, p_2)$\;
		...
		\ForEach{$v \in V$}{\If{$north = null \lor dist(v, zero) < dist(north, zero)$}{$north \leftarrow v$}}
		$V \leftarrow V \cup {zero}$\;
		\If{$north \ne null$}{$E.append(zero, north,\texttt{NORTH})$}
		...
		\For{$i\leftarrow 1$; $i \le n_{components}$; $i$\texttt{++}}{
			...
			\ForEach{$(x,y,l) \in eps$}{
				...
				\If{$best \ne null$}{$E.append(p, best,\texttt{NEIGHBOUR})$}
			}
		}
...
\begin{algorithm}
	\DontPrintSemicolon
	\caption{Supplemental functions}
	\texttt{FindKeypoints}(Image $C$, Image $S$)\;
	\Begin{
