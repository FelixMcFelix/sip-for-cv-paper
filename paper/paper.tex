...
Where $\mathcal{G}$ is indicated by context, the relevant subscripts will be elided.
...
Given a sorted label sequence $s$, we may define the \emph{exact neighbourhood} $N^{=}_{s,\mathcal{G}}(u)$ as the set of all $v \in N_\mathcal{G}(u)$ such that $seq_\mathcal{G}(u,v)=s$, and the \emph{sufficient neighbourhood} $N^{\ge}_{s,\mathcal{G}}(u)$ as the set of all $v \in N_\mathcal{G}(u)$ where we may map each label in $s$ to a distinct label of equal value in $seq_\mathcal{G}(u,v)$.
...
Transformation from an image to a graph will lead to a graph whose structure corresponds to the image's content and invariants.
If an image $I$ contains an object, it is thus reasonable to assume that the graph produced from an image of only that object should be reproduced within $I$'s graph model---such a problem might be the identification of a road sign as seen by an autonomous car.
...
The induced variant additionally imposes that $\forall u,v \in V_P, u \nsim_P v \Rightarrow i(u) \nsim_T i(v)$, ensuring that non-adjacent vertices in $P$ must remain non-adjacent in the embedding in $T$.
